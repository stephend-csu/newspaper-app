  ...
  */
  ...
  function initMap(options, chapters) {
    ...
    for (i in chapters) {
      ...
      var displayAddress = c['Chapter'].split(',')[0].trim();
      var newspapers = c['Newspapers'] ? c['Newspapers'].trim().split(/\s+/) : [];
      var newspaperContainer = $('<div class="newspaper-container"></div>');
      newspapers.forEach(function(paper) {
        if (paper) {
          newspaperContainer.append('<span class="newspaper-badge" data-paper="' + paper + '">' + paper + '</span>');
        }
      });
    ...
    // Detect unique newspapers
    var detectedNewspapers = new Set();
    chapters.forEach(function(c) {
      if (c['Newspapers']) {
        c['Newspapers'].trim().split(/\s+/).forEach(function(p) {
          if (p) detectedNewspapers.add(p);
        });
      }
    });
